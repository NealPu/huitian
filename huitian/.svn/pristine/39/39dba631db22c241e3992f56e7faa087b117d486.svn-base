			  ...
			  <div class="ibox float-e-margins">
			    <div class="ibox-title">
					<h5>
					   ...
					   &gt;  ${_res.get("report.first.menuname")}&gt;翻译报告
				   </h5>
							...
							<thead>
								...
								<c:forEach items="${showPages}" var="list" varStatus="index">
									<tr class="odd" align="center">
										...
										<c:if test="${list.issend=='0'}" >${_res.get('Unsent')}</c:if></td>
										<td>	
												<c:if test="${list.weekorday=='1' }">
												   	<a href="/report/toFixReport?pointid=${list.pointid }&type=2"  >${_res.get('Modify')} </a> &nbsp;
												   	<a href="/report/previewWeekReportUpdate/${list.tgid }" target="_blank" >${_res.get('Preview')} </a> &nbsp;
												   	<a href="/report/toPrintReportUpdate?pointid=${list.tgid }&type=2"  target="_blank" >${_res.get("Print") } </a> &nbsp;
											   	</c:if>
											   	<c:if test="${list.weekorday=='2' }">
												   	<a href="javascript:void(0)" onclick="toFixDayReport(${list.tgid})"   >${_res.get('Modify')} </a> &nbsp;
												   	<a href="/report/previewDayReportUpdate/${list.tgid}" target="_blank"  >${_res.get('Preview')}</a> &nbsp;
												   	<a href="/report/toPrintDayReportUpdate?pointid=${list.tgid }" target="_blank" >${_res.get("Print") }</a> &nbsp;
											   	</c:if>
											   	<c:if test="${list.weekorday=='1' }">
												   	 <a onclick="sendmessage(${list.tgid},1)" title="${_res.get('Sent')}" >${_res.get('Sent')} </a> &nbsp;
											   	</c:if>
											   	<c:if test="${list.weekorday=='2' }">
												   	 <a onclick="sendmessage(${list.tgid},2)" title="${_res.get('Sent')} " >${_res.get('Sent')} </a> &nbsp;
											   	</c:if>
										</td>
									</tr>
								</c:forEach>
