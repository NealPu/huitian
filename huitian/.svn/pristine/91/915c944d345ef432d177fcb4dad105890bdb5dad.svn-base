			  ...
			  <div class="ibox float-e-margins">
			    <div class="ibox-title">
				   <h5>
					...
					&gt;<a href='/sysuser/index'>机构管理</a> &gt;${_res.get('admin.user.list.table')}
				   </h5>
						...
						<table class="table table-hover table-bordered" width="100%">
							...
							<c:forEach items="${showPages.list}" var="sysuser" varStatus="status">
								<tr class="odd" align="center">
									...
									<!-- <td>
									<c:if test="${sysuser.rest_day==-1}">无</c:if>
									<c:if test="${sysuser.rest_day==1}">${_res.get('system.Monday')}</c:if>
									<c:if test="${sysuser.rest_day==2}">${_res.get('system.Tuesday')}</c:if>
									<c:if test="${sysuser.rest_day==3}">${_res.get('system.Wednesday')}</c:if>
									<c:if test="${sysuser.rest_day==4}">${_res.get('system.Thursday')}</c:if>
									<c:if test="${sysuser.rest_day==5}">${_res.get('system.Friday')}</c:if>
									<c:if test="${sysuser.rest_day==6}">${_res.get('system.Saturday')}</c:if>
									<c:if test="${sysuser.rest_day==0}">${_res.get('system.Sunday')}</c:if>
									</td> -->
									<td rowspan="${sysuser.row}">${fn:substring(sysuser.create_time,0,10)}</td>
									<td rowspan="1">${sysuser.firstRole.names}</td>
									<td align="center" rowspan="${sysuser.row}">
											<c:choose>
												...
												<c:if test="${operator_session.qx_sysusertoManageCoursePage }">
														<a href="/sysuser/toManageCoursePage?sysuserId=${sysuser.id}" title="管理用户课程">${_res.get('course.course')}</a>&nbsp;|&nbsp;
												</c:if>
												<c:if test="${operator_session.qx_sysuserfreeze }">
													<a style="color: #a4a4a4" href="javascript:void(0)" onclick="freezeAccount(${sysuser.id},1)">${_res.get('Suspending')}</a>&nbsp;|&nbsp;
												</c:if>
												<%-- <c:if test="${operator_session.qx_operatorsetSysUserGroup }">
													<a href="javascript:void(0)" onclick="setSysUserGroup(${sysuser.id})">分组</a>
												</c:if> --%>
												<c:if test="${operator_session.qx_operatorsetSysUserGroup }">
