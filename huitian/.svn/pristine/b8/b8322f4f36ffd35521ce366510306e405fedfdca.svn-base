				...
				<form action="" method="post" id="studentForm" enctype="multipart/form-data">
					...
					<fieldset>
						...
						<p class="" style="width: 400px">
							...
							<input type="radio" name="student.sex" value="1" <c:if test="${student.sex}">checked="checked"</c:if> />${_res.get('student.boy')} <input type="radio" name="student.sex" value="0" <c:if test="${student.sex == false }">checked="checked"</c:if> /> ${_res.get('student.girl')}
						</p>
						<p class="">
