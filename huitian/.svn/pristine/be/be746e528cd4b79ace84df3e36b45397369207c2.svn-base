...
public class AccountService extends BaseService {
	...
	private AccountBookService accoubtBookService = new AccountBookService();
	private CourseOrderService courseOrderService = new CourseOrderService();

	public List<Account> getTeacherByCourseId(String courseId) {
		List<Account> list = new ArrayList<Account>();
		List<Account> lists = dao.getTeachers();
		for (Account acc : lists) {
			String classType = acc.getStr("CLASS_TYPE");
			String[] type = classType.replace("|", ",").split(",");
			circle: for (int i = 0; i < type.length; i++) {
				if (type[i].equals(courseId)) {
					list.add(acc);
					break circle;
				}
			}
	...
	@Before(Tx.class)
	public synchronized Map<String, String> accountManage(String type, String accountId, String paymentId, String orderId, String operateId,Double amount, String courseplanId,String banciId) {
		...
		if (StringUtils.isEmpty(accountId)) {
			...
			Account account = dao.findById(Integer.parseInt(accountId));
			if (account == null) {
				msg = "账户不存在";
				log.info("账户ID：" + accountId + "不存在");
			} else {
				...
				if (StringUtils.isEmpty(operateId)) {
					...
					if (sysuser == null) {
								...
								if (payment == null) {
									...
									if (courseOrder == null) {
										...
										if (total <= realsum) {
											...
											accoubtBookService.save(accountBook);
											if (total == realsum) {
												...
												if (rebatefee > 0) {
													...
													accoubtBookService.save(_accountBook);
												}
						...
						case Constants.ACCOUNT_ZHUANCUN:
							...
							accoubtBookService.save(_accountBookZhuancun);
							account.set("update_time", new Date());
							account.update();
							code = "1";
							break;
						case Constants.ACCOUNT_ZHUANRU:
							...
							if(totaltemp>=amount){//够转
								...
								accoubtBookService.save(_accountBookZhuanru);
		...
		if(carriedOver == 0){//排课先添加一条未结转消耗记录
			for(CourseOrder courseOrder : courseOrderList){
				...
				if(ToolArith.compareTo(consumeClassHour, orderClasshour)==0){//说明已消耗完
					...
					Record deducationRecord = AccountBook.dao.getDeductionFee(coursePlanId);//已消耗的费用
