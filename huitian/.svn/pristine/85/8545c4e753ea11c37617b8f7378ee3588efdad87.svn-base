					...
					for(var j=0;j<date.dayLists.length;j++){
						str += "<td id=\""+k+"_"+date.dayLists[j]+"\" >";
			      	     for(var dl=0;dl<date.courseMap[k][date.dayLists[j]].length;dl++){
			      	    	...
			      	    	+date.courseMap[k][date.dayLists[j]][dl].RANK_NAME+"</label><span class='delcol' onclick='delCoursePlan("
			      	    			+date.courseMap[k][date.dayLists[j]][dl].CPID+",\""
			      	    			+date.courseMap[k][date.dayLists[j]][dl].COURSE_TIME+"\");'>X</span><br>";
			      	    	str += "<label class='fontweig'>"+date.courseMap[k][date.dayLists[j]][dl].TCHNAME+"</label><br><label class='fontweig'>" 
			      	    	+ date.courseMap[k][date.dayLists[j]][dl].CAMPUS_NAME+ "--"+date.courseMap[k][date.dayLists[j]][dl].ROOMNAME +"</label></div>";
			      	     }
		...
		success : function(data) {
			if(data.code=="0" ){//课时够用
				 $.layer({
				    ...
				    border: [0],
			        area: ['650px', '600px'],
			        iframe: {src: '/course/toArrangeCoursePlan?courseid='+courseid+"&daytime="+daytime+"&stuid="+studentId}
				 });
			}else{
								...
								<p>
									...
									<a href="#" id="search" class="dengemail" onclick="getCalender()" style="padding: 8px;">${_res.get('admin.common.determine')}</a>
									 ...
									 <label> ${_res.get('View.Teacher.Timetable')}： </label> 
										...
										</select> <input id="tids" name="allteacherids" value="" type="hidden">&nbsp;&nbsp;
