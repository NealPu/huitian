...
/* 	$().ready(function() {
		if ("${account_session.user_type}" == "2" || "${account_session.user_type}" == "1") {//角色登录是否显示搜索框 如果是老师或者学生
			$("#searchTable").hide();
		} else {
			$("#searchTable").show();
		}
					...
					if(result.nums=="1"){
						$.layer({
							...
							$.layer({
						        ...
						        area : ['380px' , '430px'],
						        offset : ['80px', ''],
						        iframe: {src: "/course/fullHourDelCoursePlan/"+planId+',1'}
						    });
					}
				}
			});
...
<style type="text/css">
 ul li{display: block;padding:5px}
 .navbar ul li{padding:0}
 .component {line-height: 1.5em;padding: 2em 0 3em;width: 100%;width: 1070px;overflow: hidden}
 .component .filler {color: #d3d3d3}
 table {border-collapse: collapse;margin-bottom: 3em;background: #fff}
 td, th {padding: 2px}
...
/* Not needed for sticky header/column functionality */
 ...
 #laydate_table > thead > tr >th{background-color:#fff !important;color:#00625A !important;}
 ul li{padding:0}
 .hid{
 	display: none;
 }
		...
		<div class="ibox float-e-margins" style="width:1070px">
		    <div class="ibox-title">
				<h5>
					...
					&gt; ${_res.get('curriculum_management')}
				</h5>
					...
					<fieldset id="searchTable">
					  <div class="form-group" style="margin-top: 15px;">
					     <div style="float: left;">
							<label>${_res.get('student')}：</label> <input type="text" name="studentName" id="studentName" value="${studentName}" style="border: 1px solid #bababa;width:130px;height: 34px;" /> 
							<label>${_res.get('teacher')}：</label> <input type="text" name="teacherName" id="teacherName" value="${teacherName}" style="border: 1px solid #bababa;width:130px;height: 34px;" /> 
							<label>${_res.get('classNum')}：</label> <input type="text" name="banci" id="banci" value="${banci}" style="border: 1px solid #bababa;width:130px;height: 34px;" />
						 </div>
						 <div style="float: left;">	
							<label style="float: left;margin:9px 0 0 2px">${_res.get('course.class.date')}：</label>
							<div style="float: left;">	
							   <input type="text" class="form-control layer-date" readonly="readonly" id="date1" name="date1" value="${date1}" style="width:160px;background-color: #fff;" />
							</div>
							<div style="width:30px;height:30px;line-height:30px;text-align:center;background:#E5E6E7;float: left;margin-top:1px">至</div>
							<div style="float: left;">	    
							    <input type="text" class="form-control layer-date" readonly="readonly" id="date2" name="date2" value="${date2}" style="width:160px;background-color: #fff;" />
						    </div>
						</div>
						<div style="clear: both;"></div>	
							...
							<c:if test="${operator_session.qx_coursedeleteCoursePlans}">
								<div>
									...
									<div  name="showDeleteMessage" class="hid" style="float:right;">
										<input type="button" value="${_res.get('Remove_the_radio')}" onclick="DeleteOne()" class="btn btn-outline btn-info">
										<input type="button" value="${_res.get('Confirm_deletion')}" onclick="isDeleteMultipleChoice()" class="btn btn-outline btn-info">
									</div>
								</div>
							</c:if>
							<c:forEach items="${timeMap }" var="courseDate" varStatus="tm">
								<th id="col${tm.count }" style=""  valign="top">${courseDate.key }
										<script type="text/javascript">
						...
						<c:forEach items="${timeMap }" var="courseDate" varStatus="tm">
							<td style="background:#f3f3f4"  valign="top">
								...
								<c:forEach items="${courseDate.value }" var="course" varStatus="cId" >
									<li style="background:#D9EDF7;color:#676A6C;margin:3px 0;padding:5px;min-width:140px" class="plantype_${course.PLAN_TYPE }"  id="planDay_${course.ROOM_ID}_${courseDate.key}_${course.RANK_ID}_${course.CAMPUS_ID}">
										<c:if test="${operator_session.qx_coursedelCoursePlan }">
											...
											<div name="hideDeleteMessage" class="">
												<a href='javascript:void(0)' style='vertical-align: middle; display: inline-block; float: right;'
														onclick="delKeng('${course.ID}','planDay_${course.ROOM_ID}_${courseDate.key}_${course.RANK_ID}_${course.CAMPUS_ID}',this);"><img
														src='/images/img/error.png' width='12' height='12' title='取消' />
												</a>
											</div>
										</c:if>
										 <c:choose>
										 	...
										 	<c:otherwise>
												...
												${_res.get("classroom")}:${course.CAMPUS_NAME}-${course.ROOM_NAME}<br>
											 	...
											 	<c:if test="${empty course.T_NAME}">--${_res.get('mock.test')}--<br></c:if>
												${_res.get('course.course')}:${course.COURSE_NAME}<c:if test="${course.rechargecourse }">(${_res.get('Complement.row')})</c:if><br>
											 	<c:if test="${!empty course.T_NAME}">${_res.get('teacher')}:${course.T_NAME}<br></c:if>
											 	<c:choose>
