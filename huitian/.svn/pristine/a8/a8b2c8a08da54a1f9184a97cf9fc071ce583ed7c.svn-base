...
public class ClassType extends BaseModel<ClassType> {
	...
	public List<ClassType> getClassTypekeyong() {
		String sql = "SELECT *  FROM class_type WHERE status = 1";
		List<ClassType> list = ClassType.dao.find(sql);
		return list;
	}
	...
	public List<Record> getClassCoursePlans(Integer class_id) {
		String sql = "select * from courseplan where COURSE_TIME > CURDATE() AND CLASS_ID = ?";
	...
	public Record getClassOrderbyid(String orderid) {
		String sql = "select * from class_order where id=? ";
