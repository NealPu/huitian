				...
				<div class="zsd-cont-r">
					...
					<div class="ibox-content" id="jiaowu_pingjia" style="margin-bottom: 20px">
						<form action="" method="post">
							...
							<textarea readonly="readonly" style="background: #f2f2f2; border-radius: 6px; border: 1px solid #e2e2e2; padding: 3px; width: 100%;" rows="5">${t.COURSE_DETAILS}</textarea></li>
							<li><span>${_res.get('This.job')}:</span> 
							<textarea readonly="readonly"  style="background: #f2f2f2; border-radius: 6px; border: 1px solid #e2e2e2; padding: 3px; width: 100%;" rows="5">${t.HOMEWORK}</textarea></li>
							<li><span>${_res.get("qs.in.homework")}:</span> 
							<textarea readonly="readonly" style="background: #f2f2f2; border-radius: 6px; border: 1px solid #e2e2e2; padding: 3px; width: 100%;" rows="5">${t.QUESTION}</textarea></li>
							<li><span>${_res.get("solutions")}:</span> 
							<textarea readonly="readonly" style="background: #f2f2f2; border-radius: 6px; border: 1px solid #e2e2e2; padding: 3px; width: 100%;" rows="5">${t.METHOD}</textarea></li>
							<li><span>${_res.get("course.remarks")}/${_res.get("reminder")}:</span> 
							<textarea readonly="readonly" name="teachergrade.tutormsg" style="background: #f2f2f2; border-radius: 6px; border: 1px solid #e2e2e2; padding: 3px; width: 100%;" rows="5"></textarea>${t.TUTORMSG}</li>
							<hr>
							<div>
								<h5>教师课堂表现：</h5>
							...
							<ol class="teacher-comment-sele" style="padding: 0">
								<li>
									...
									<div class="col-lg-3" style="margin: 10px 0;">
										...
										<select  id="shangcizuoye">
											<option ${t.QHL=="优"?"selected='selected'":""} value="优">--${_res.get("Not_furnished")}--</option>
											<option ${t.QHL=="良"?"selected='selected'":""} value="良">--${_res.get("Well_completion")}--</option>
											<option ${t.QHL=="中"?"selected='selected'":""} value="中">--${_res.get("Completion_poor")}--</option>
											<option ${t.QHL=="差"?"selected='selected'":""} value="差">--${_res.get("incomplete")}--</option>
										</select>
									</div>
									<span>${_res.get("course.remarks")}:</span> 
									<textarea id="tutormsg" style="background: #f2f2f2; border-radius: 6px; border: 1px solid #e2e2e2; padding: 3px; width: 100%;" rows="5">${t.SBZ}</textarea>
