				...
				<table class="table table-hover table-bordered">
					...
					<c:forEach items="${paymentList}" var="payment" varStatus="index">
						<tr align="center">
							...
							<td>${payment.ispay eq true?'已收':'未收'}</td>
							<td>
								<c:choose>
									<c:when test="${payment.paytype eq 1 }">现金</c:when>
									<c:when test="${payment.paytype eq 2 }">银行转账</c:when>
									<c:when test="${payment.paytype eq 3 }">POS机</c:when>
									<c:otherwise>${_res.get('Else')}</c:otherwise>
								</c:choose>
							</td>
							<td>${payment.confirmusername}</td>
