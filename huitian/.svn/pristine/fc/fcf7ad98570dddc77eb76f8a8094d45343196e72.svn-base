...
public class CoursePlan extends BaseModel<CoursePlan> {
	private static final long serialVersionUID = 5563173819853189585L;
	public static final CoursePlan coursePlan = new CoursePlan();
	...
	public static String getNames(String roomName,Integer rId,String courseCampusName,String courseDate){
		...
		for(CoursePlan cp : list){
			...
			h++;
			if(!cp.getInt("STATE").toString().equals("2")){
				str.append(cp.getStr("REAL_NAME"));
			}
		}
		return str.toString();
	}
	 ...
	 * 根据日期段查询期间有课的日期
	 */
	public static List<CoursePlan> getCourseDate(String date1,String date2){
		String sql = "SELECT " +
...
//		return cp.getLong("count_course").intValue();
//	}
	public static Integer getUseCourse(Integer stu_id,Integer tac_id) {
		String sql = "SELECT COUNT(*) AS count_course from courseplan WHERE STUDENT_ID=? and TEACHER_ID=? and state=0 and PLAN_TYPE=0 ";
		CoursePlan cp = CoursePlan.coursePlan.findFirst(sql, stu_id,tac_id);
		return cp.getLong("count_course").intValue();
	}
	...
	public List<CoursePlan> getStuCoursePlansBetweenDates(String stuId, String startDate, String endDate) {
		String sql = "select cp.ID,cp.class_id, cp.STUDENT_ID, cp.COURSE_ID , course.COURSE_NAME,"
				...
				+ " cp.ROOM_ID,room.NAME,DATE_FORMAT(cp.COURSE_TIME,'%Y-%m-%d') AS COURSE_TIME,cp.iscancel "
	...
	public List<CoursePlan> getCoursePlansByDay(String cpday, String stuId) {
		String sql = "select cp.ID, cp.STUDENT_ID,cp.TIMERANK_ID,"
				+ " tr.RANK_NAME,DATE_FORMAT(cp.COURSE_TIME,'%Y-%m-%d') AS COURSE_TIME,cp.ROOM_ID,cp.CAMPUS_ID from courseplan as cp "
				+ "left join time_rank as tr on cp.TIMERANK_ID=tr.ID "
	...
	public List<CoursePlan> getCoursePlansByTeacherId(String tId, String startDate, String endDate) {
		String sql = "select cp.ID, cp.STUDENT_ID, cp.COURSE_ID,cp.class_id,co.CLASSNUM,"
				...
				+ " room.NAME,DATE_FORMAT(cp.COURSE_TIME,'%Y-%m-%d') AS COURSE_TIME ,cp.iscancel "
				+ " from courseplan as cp "
				+ "left join course on cp.COURSE_ID=course.ID "
				+ "left join account as teacher on cp.TEACHER_ID=teacher.ID "
				+ "left join time_rank as tr on cp.TIMERANK_ID=tr.ID "
	...
	public double getUsedVIPClasshour(Integer studentId,Integer subjectId) {
		String sql = "select IFNULL(SUM(classhour),0) ks from account_book where courseorderid IN (select id from crm_courseorder where studentid=? and teachtype=1 and `status`!=0 AND FIND_IN_SET(?,REPLACE(subjectids,'|',','))) and operatetype=4"; 
		Double sumhour = Db.queryDouble(sql.toString(),studentId,subjectId);
		return sumhour==null?0:sumhour;
	}
	 ...
	 */
	public boolean checkSubjectIsUse(Integer subjectId,Integer studentId) {
		String sql = "SELECT COUNT(1) FROM courseplan WHERE STUDENT_ID=? AND SUBJECT_ID=?";
		return Db.queryLong(sql,studentId,subjectId)==0?false:true;
	}
...
//		return keshi;
	 ...
	 * @return
	 */
	public double monthkeshi(String date1, String date2, Integer integer) {
	 ...
	 * @return
	 */
	public List<CoursePlan> getStudentDayCoursePlans(String stuId, String courseid, String day) {
		String sql = " select cp.Id cpid,cp.course_time,tr.id trid,tr.rank_name,campus.id campusid,campus.campus_name,tch.id tchid,tch.real_name tchname,room.id roomid,room.name roomname from courseplan cp left join time_rank tr on tr.id=cp.timerank_id "
				+ " left join campus on campus.id=cp.campus_id left join account tch on tch.id=cp.teacher_id left join classroom room on room.id=cp.room_id where cp.student_id = ? and cp.course_id= ? and course_time = ? ";
		return coursePlan.find(sql, stuId, courseid,day);
	}
	...
	public List<CoursePlan> findByCampusidAndTeacherids(String campusid, String ids, String date,Integer teacherid) {
		...
		sf.append("SELECT cp.Id cpid,cp.TIMERANK_ID,cp.class_id, s.id studentid,t.real_name teachername,s.real_name studentname "
				...
				+ " LEFT JOIN account t ON cp.TEACHER_ID = t.Id "
				+ " LEFT JOIN account s ON cp.STUDENT_ID = s.id "
				+ " where cp.STATE = 0 and DATE_FORMAT(cp.COURSE_TIME,'%Y-%m-%d') = '"+date+"' and t.id = "+teacherid);
		if(!campusid.equals("")){
	...
	public List<CoursePlan> getAllCoursePlan(Map<String, String> queryParams) {
		if(queryParams == null){
			...
			sql.append("select concat(" +
					"IF(courseplan.iscancel=1,'(已取消)<br>','')," +
					"IF(courseplan.PLAN_TYPE=1,'类型:模考<br>',IF(courseplan.PLAN_TYPE=2,'类型:休息',IF(courseplan.PLAN_TYPE=0 AND courseplan.class_id=0,IF(courseplan.netCourse=1,'授课:一对一(网络课)<br>','授课:一对一<br>'),'')))," +
					"IF(ISNULL(course.course_name),'',CONCAT('课程:',course.course_name))," +
					"IF(courseplan.rechargecourse=1,'(补排)<br>','<br>')," +
					"IF(ISNULL(teacher.real_name),'',CONCAT('老师:',teacher.real_name,'<br>'))," +
					"IF(courseplan.PLAN_TYPE!=2,CONCAT('场地:',IFNULL(campus.campus_name,''),classroom.name,'<br>日期:',date_format(courseplan.course_time,'%Y-%m-%d'),'<br>时段:',time_rank.RANK_NAME,IF(courseplan.class_id!=0,'',CONCAT('<br>学生:',stu.real_name))),CONCAT( '时段:',IF(LENGTH(courseplan.startrest)<=4,CONCAT('0',courseplan.startrest),courseplan.startrest), '-', courseplan.endrest ))," +
					"IF(courseplan.class_id!=0,CONCAT('<br>班型:',xiaoban.bantypename,'<br>班次:',xiaoban.classNum),'')) as  title," +
					...
					"courseplan.student_id,courseplan.course_id, courseplan.class_id, courseplan.remark, date_format(courseplan.course_time,'%Y,(%c-1),%d') as start," +
					...
					"date_format(courseplan.course_time,'%Y-%m-%d') kcrq "+
						...
						"left join " +
						"(select bc.id,bx.`name` bantypename,bc.classNum from class_order bc left join class_type bx on bc.classtype_id=bx.id) xiaoban " +
						"on xiaoban.id = courseplan.class_id " +
						"left join account stu on stu.id=courseplan.student_id " +
						"left join account teacher on teacher.id=courseplan.teacher_id ");
			sql.append(" where courseplan.course_time>='").append(queryParams.get("startDate")).append("' and courseplan.course_time<= '").append(queryParams.get("endDate")).append("' ");
			
			if(!StringUtils.isEmpty(queryParams.get("studentId"))){//学生登录
				Integer studentId = Integer.parseInt(queryParams.get("studentId"));
				sql.append(" and stu.id=").append(studentId).append(" or xiaoban.id in(SELECT GROUP_CONCAT(ab.banci_id) from account_banci ab where ab.account_id=").append(studentId).append(")");
			}
...
//			StringBuffer timeids = TimeRank.dao.getTimeIdsByTime(queryParams.get("starthour"), queryParams.get("startmin"), queryParams.get("endhour"), queryParams.get("endmin"));
//			String sbStr = timeids.toString();
//			if (!ToolString.isNull(sbStr)) {
//				timeids.insert(0, "(");
//				timeids.append(")");
//				sql.append(" and time_rank.id in ( ").append(sbStr).append(" ) ");
//			}
			List<CoursePlan> result = coursePlan.find(sql.append(") a ORDER BY a.course_time asc, a.trrankname asc").toString());
			return result;
		}
	}
