...
<body>
	...
	<script type="text/javascript">
		...
		function getCalender() {
			...
			var campus = $("#xqid").val();
			if (stuId != "") {
				if ((course_type == 0 && teacherId != 0 && campus != 0) || (course_type == 1 && classtype != 0 && campus != 0)) {
					$
							.ajax({
								...
								success : function(date) {
									if (date.result != "noResult") {
										...
										for (var j = 0; j < date.result.timerank.length; j++) {
											...
											strArray.push("</th>");
											for (var z = 0; z < date.result.dayLists.length; z++) {
												/*需要更改id*/
				...
				} else if (course_type == 0 && (teacherId == '0' || campus == '0' || teacherId == '')) {
					alert("请选择课程丶老师和校区！");
