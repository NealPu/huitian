...
<body id="body" style="min-width: 1100px;">
	<div id="wrapper" style="background: #2f4050; height: 100%;">
		...
		<div class="gray-bg dashbard-1" id="page-wrapper" style="height: 100%">
			...
			<div class="margin-nav2">
				<form action="/student/index" method="post" id="searchForm">
          		...
          		<div style="clear:both"></div>
							...
							<div class="ibox-content">
								...
								<label>所属班次： </label> 
								<select name="_query.classorderid" id="classorderid" class="chosen-select" style="width: 120px;" tabindex="2">
									<option value="">${_res.get('system.alloptions')}</option>
								...
								<table class="table table-hover table-bordered">
									<thead>
										<tr align="center">
											...
											<th rowspan="2" style="height: 15px; line-height: 34px;">${_res.get("student.buildtime") }</th>
											<th colspan="5" style="height: 15px; line-height: 15px; border-bottom-width: 0; padding: 2px 0;">${_res.get("IEP")}(${_res.get("session")})</th>
											<th rowspan="2" style="height: 15px; line-height: 34px;">${_res.get('classNum')}</th>
											<th rowspan="2" style="height: 15px; line-height: 34px;">${_res.get("marketing.specialist")}</th>
											<th rowspan="2" style="height: 15px; line-height: 34px;">${_res.get("Their.academic")}</th>
											<th rowspan="2" style="height: 15px; line-height: 34px;">${_res.get("course.consultant")}</th>
										...
										<tr style="font-size: 10px; line-height: 15px;">
											...
											<th style="height: 15px; line-height: 15px; font-weight: normal; padding: 2px 0;">未排</th>
										</tr>
									</thead>
									<c:forEach items="${showPages.list}" var="student" varStatus="index">
										...
										<tr align="center">
											...
											<td>${student.zksvip==null?'0':student.zksvip}</td>
											<td>${student.ysksvip==null?'0':student.ysksvip}</td>
											<td ${student.zksvip-student.ysksvip<=4?"style='background-color: #FFF68F;'":""}>${student.zksvip-student.ysksvip}</td>
											<td>${student.ypksvip==null?'0':student.ypksvip}</td>
											<td ${student.zksvip-student.ypksvip<=4?"style='background-color: #FFF68F;'":""}>${student.zksvip-student.ypksvip}</td>
											<td>
												...
												<c:choose>
													...
													<c:otherwise>
														<c:if test="${operator_session.qx_studentedit }">
															<a href="javascript:void(0)" onclick="window.location.href='/student/edit/'+${student.id}" >${ _res.get('admin.common.edit')}</a>
														</c:if>
														<c:if test="${operator_session.qx_studentgoBuyCoursePage }">
															<a href="javascript:void(0)" onclick="buyCourse(${student.id})">${_res.get('Purchase')}</a>
														</c:if>
														<c:if test="${operator_session.qx_studentshowCourseUsedCount }">
															<a href="/student/showCourseUsedCount/${student.id}" title="${_res.get('courses_statistics')}">${_res.get('add.up')}</a>
														</c:if>
														<c:if test="${operator_session.qx_studentgetStudentCourse }">
															<a href="javascript:void(0);" onclick="getStudentCourse(${student.id})">${_res.get('course.course')}</a>
														</c:if>
														<c:if test="${operator_session.qx_studentpassword }">
															<a href="javascript:void(0)" onclick="changePassword(${student.id})" title="${_res.get('Change.password')}">${_res.get("passWord")}</a>
														</c:if>
														<c:if test="${operator_session.qx_teacherfreezestu }">
															<a href="javascript:void(0)" onclick="freezeAccount(${student.id},1)">${_res.get('Suspending')}</a>
														</c:if>
														<c:if test="${operator_session.qx_courseaddCourseWeekPlan }">
															<a href="javascript:void(0)" onclick="window.location.href='/course/addCourseWeekPlan?studentId=${student.id}'">${_res.get('course.arranging')}</a>
														</c:if>
													</c:otherwise>
												</c:choose>
											</td>
    ...
    function buyCourse(id){
    	...
    	window.location.href='${cxt}/orders/goBuyCoursePage/'+id;
    }
