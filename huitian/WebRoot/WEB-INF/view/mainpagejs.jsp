...
/*获取学生课程信息*/
		...
		$(document).ready(function(){
			...
			queryCourseSaleRecords();
			queryCourseCommentRecords();
			queryCourseQARecords();
		} );
		
		
		/* 网校课程销售情况 */
		function queryCourseSaleRecords() {
			$.ajax( {
				url : "/main/queryUserNetAccountId",
				dataType : "json",
				type : "post",
				async : false,
				success : function( result ) {
					if( result.flag ) {
						$.ajax( {
							...
							data : { 
								...
								"netUserId" : result.netUserId 
							},
							dataType : "json",
							type : "post",
							async : false,
							success : function( sourceResult ) {
								var sourceSize = sourceResult.list.length;
								if( source.list != null && sourceSize > 0 ) {
									var saleSourceStr = "";
									for( var i = 0 ; i < sourceSize ; i++ ) {
										saleSourceStr += " <tr><td> " + i + "</td>";
										saleSourceStr += " <td> " + sourceResult.list[i].COURSENAME + "</td>";
										saleSourceStr += " <td> " + sourceResult.list[i].SALESCOUNT + "</td>";
										saleSourceStr += " <td> <a class='btn btn-xs btn-primary'>查看</a> </td></tr>";
									} 
									$( "#courseSaleRecords" ).html( saleSourceStr );
								}
							}
						} );
					}
		...
		/* 网校课程销售情况 */
		function queryCourseCommentRecords() {
			$.ajax( {
				url : "/main/queryUserNetAccountId",
				dataType : "json",
				type : "post",
							...
							success : function( sourceResult ) {
								...
								if( source.list != null && sourceSize > 0 ) {
									...
									for( var i = 0 ; i < sourceSize ; i++ ) {
										saleSourceStr += " <tr><td> " + i + "</td>";
										saleSourceStr += " <td> " + sourceResult.list[i].COURSENAME + "</td>";
										saleSourceStr += " <td> " + sourceResult.list[i].COMMENT + "</td>";
										saleSourceStr += " <td> <a class='btn btn-xs btn-primary'>查看</a> </td></tr>";
									} 
									$( "#courseCommentRecords" ).html( saleSourceStr );
								}
							}
						} );
					}
							...
							success : function( sourceResult ) {
								...
								if( source.list != null && sourceSize > 0 ) {
									...
									for( var i = 0 ; i < sourceSize ; i++ ) {
										saleSourceStr += " <tr><td> " + i + "</td>";
										saleSourceStr += " <td> " + sourceResult.list[i].COURSENAME + "</td>";
										saleSourceStr += " <td> " + sourceResult.list[i].QUESTION + "</td>";
										saleSourceStr += " <td> <a class='btn btn-xs btn-primary'>查看</a> </td></tr>";
									} 
									$( "#courseQARecords" ).html( saleSourceStr );
								}
							}
						} );
					}
