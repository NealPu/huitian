		...
		<div class="ibox-content" style="padding-bottom: 0">
			<form action="" method="post" id="teacherForm">
				...
				<fieldset>
					...
					<p>
						<label> ${_res.get('job.category')}： </label> <input type="radio" name="teacher.tworktype" value="1" <c:if test="${teacher.tworktype=='1'}">checked="checked" </c:if> checked="checked" /> ${_res.get("Full-time")} <input type="radio" name="teacher.tworktype" value="0" <c:if test="${teacher.tworktype=='0'}">checked="checked" </c:if> /> ${_res.get("Part-time.job")}
						</td>
					</p>
					<p>
						<label>${_res.get("If.a.foreign.teacher")}</label> <input type="radio" name="teacher.isforeignteacher" <c:if test="${teacher.isforeignteacher==1}">checked="checked" </c:if> value="1" />${_res.get("admin.common.yes")} <input type="radio" name="teacher.isforeignteacher" <c:if test="${teacher.isforeignteacher==0}">checked="checked" </c:if> value="0" /> ${_res.get("admin.common.no")}
					</p>
					<p>
						<label>是否接收短信： </label> <input id="receiveSmsTeacher" type="radio" name="teacher.receive_sms_teacher" value="1" ${teacher.receive_sms_teacher==null?"checked=checked'":(teacher.receive_sms_teacher==1?"checked=checked'":"") } /> ${_res.get('admin.common.yes')} <input id="receiveSmsTeacher" type="radio" name="teacher.receive_sms_teacher" value="0" ${teacher.receive_sms_teacher==0 ? "checked=checked'":"" } /> ${_res.get('admin.common.no')}
					</p>
					<p>
