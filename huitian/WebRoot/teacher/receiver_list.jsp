...
<body id="body" style="min-width:1100px;">   
	<div id="wrapper" style="background: #2f4050;height:100%;">
	 ...
	 <div class="gray-bg dashbard-1" id="page-wrapper" style="height:100%">
		...
		<form action="/teacher/queryAllReceiver" method="post" id="searchForm" >
			  ...
			  <div class="ibox float-e-margins">
			    <div class="ibox-title">
					<h5>
						...
						&gt;<a href='/teacher/queryAllAnnouncement'>消息中心</a> &gt;已收消息
				   </h5>
				...
				<div class="ibox-content">
					<%-- <label>发送人：</label>
					<input type="text" id="studentname" name="_query.fsrname" style="width:150px;" value="${paramMap['_query.fsrname']}"> --%>
					<label>标题：</label>&nbsp;
					<input type="text" id="phonenumber" name="_query.title" style="width:150px;" value="${paramMap['_query.title']}">
					<label>发送日期：</label>&nbsp;
					...
					<label>阅读状态： </label>
					<select name="_query.states"  class="chosen-select" style="width:100px;" tabindex="2">
						<option value="">全部</option>
						<option value="0" <c:if test="${0==paramMap['_query.states']}">selected=""selected</c:if>>未读</option>
						<option value="1" <c:if test="${1==paramMap['_query.states']}">selected=""selected</c:if>>已读</option>
					</select>
				 	<input type="button" onclick="search()" value="${_res.get('admin.common.select') }" class="btn btn-outline btn-info">
			     	<input type="button" onclick="updateStates()" class="btn btn-outline btn-info" value="${_res.get('Mark_all_as_read')}">
			      <div style="clear: both;"></div>
			   </div>
			 </div>
		   </div>
					...
					<div class="ibox-content">
						<table class="table table-hover table-bordered">
							...
							<tr align="center">
								...
								<th style="height:15px;line-height: 34px;">发送人</th>
								<th style="height:15px;line-height: 34px;">标题</th>
								<th style="height:15px;line-height: 34px;">发送时间</th>
								<th  style="height:15px;line-height: 34px;">阅读状态</th>
								<th style="height:15px;line-height: 34px;">${_res.get('operation')}</th>
							</thead>
							<c:forEach items="${showPages.list}" var="r" varStatus="index">
								<tr align="center">
									<td>${index.count }</td>
									<td>${r.REAL_NAME}</td>
									<td>${r.title}</td>
									<td>
										<fmt:formatDate value="${r.sendtime}" type="time" timeStyle="full" pattern="yyyy-MM-dd hh:mm:ss"/>
									</td>
									<td>${r.state==0?'未读':'已读'}</td>
									<td align="center">
										<a href="javascript:void(0)" onclick="viewMessage(${r.arid})" >详情</a>&nbsp;|
										<a href="javascript:void(0)" onclick="replyMessage(${r.arid})" >回复</a>
									</td>
								</tr>
							</c:forEach>
						</table>
...
<!-- Chosen -->
    ...
    function viewMessage(id){
    	$.layer({
    		...
    		border:[0],
    		area:['900px','620px'],
    		iframe:{src:'${cxt}/teacher/viewReceiver/'+id}
    	});
    }
