...
<body>
            ...
            <form action="" method="post" id="documentForm"  enctype="multipart/form-data">
					<fieldset>
						<p>
							...
							<input type="text" name="documentname" id="documentname" value=""  style="font-size:14px"/>
						</p>
						<p >
							...
							<select name="studentid" id="studentid"  >
								...
								<c:forEach items="${stulist }" var="stu" >
									<option value="${stu.Id }" >${stu.real_name }</option>
								</c:forEach>
							</select>
						</p>
						<p>
							<label>${_res.get('Document.Type')}：</label> 
							<input type="text" name="contenttype" id="contenttype" value=""  style="font-size:14px"/>
						</p>
						...
						　　　　　　　　<input type="text" id="PathDisplayer" name="PathDisplayer" placeholder="${_res.get('Click.Choose.File')}"  class="input-text" disabled /> 
						　　　　　　　　<a href="javascript:void(0)" class="btn btn-sm btn-warning link-btn" >${_res.get('Select.the.file')}</a> 
						　　　　　　</div> 
						<span style="color:red">文档最大不能超过10M。</span>
						　　　　</div>
						</p>
						<input type="button" value="${_res.get('Upload')}" onclick="uploadFiles()" class="btn btn-outline btn-info">
					</fieldset>
				</form>
          </div>
	   </div>
    ...
    <script src="/js/webuploader/ajaxfileupload.js"></script>
    <script>
        //弹出后子页面大小会自动适应
        ...
        function uploadFiles(){
        	if($("#PathDisplayer").val().trim()==""||$("#PathDisplayer").val()==null){
				alert("请选择文件");
				return false;
			}
			 ...
			 $.ajaxFileUpload({
				...
				url:"/document/uploadThisDocument",
				data:{
