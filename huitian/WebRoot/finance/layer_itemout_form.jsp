		...
		<div style="height:240px">
			<div style="margin-top: 10px;padding:15px;background:#fff">
				<form id="itemsForm" action="${cxt }/itemsManager/saveItemsIn" method="post">
					<fieldset>
						<input type="hidden" id="handletype" name="itemsOutIn.handletype" value="fromstorage" >
						...
						<p>
							<label>单价(含税)：</label>
							<input type="text" id="handlenum" name="itemsOutIn.ftaxprice" value="${items.ftaxprice }" size="20" />
						</p>
						<%-- <p>
							<label>税率(%)：</label>
							<input type="text" id="handlenum" name="itemsOutIn.ftaxrates" value="${items.ftaxrates }" size="20" />
						</p>
						<p>
							<label>单价(不含税)：</label>
							<input type="text" id="handlenum" name="itemsOutIn.fprice" value="${items.fprice }" size="20" />
						</p> --%>
						<p>
							<label>数量：</label>
							<input type="text" id="handlenum" name="itemsOutIn.fnum" value="${items.fnum }" size="20" />
						</p>
						<p>
							<label>出库总额<!-- (不含税) -->：</label>
							<input type="text" id="handlenum" name="itemsOutIn.ftotalcost" value="${items.ftotalcost }" size="20" />
						</p>
		...
		function save() {
			...
			if ($("#handlenum").val() == "" || $("#handlenum").val() == null) {
				...
				if(operatorType=="add"){
					$.ajax({
                    	type:"post",
						url:"${cxt}/itemsManager/saveItemsOut",
					...
					if(confirm("确定要修改该出库信息吗？")){
						 $.ajax({
	                        	type:"post",
								url:"${cxt}/itemsManager/updateItemsOut",
								data:$('#itemsForm').serialize(),
								datatype:"json",
								success : function(data) {
									 if(data.code=='0'){
