...
<html>
<head>
<title>${_res.get('Teacher.Attendance')}</title>
...
<body>
	<div id="wrapper" style="background: #2f4050;min-width:1100px">
		...
		<div class="gray-bg dashbard-1" id="page-wrapper">
			...
			<div class="margin-nav" style="width: 100%;">
				<form action="/teacherchecke" method="post" id="searchForm">
					<div class="col-lg-12">
						<div class="ibox float-e-margins">
						    <div class="ibox-title">
								<h5>
								     ...
								     &gt;<a href='/attendance/studentIndex'>考勤管理</a> &gt;  ${_res.get('Teacher.Attendance')}
							   </h5>
							</div>
								...
								<table class="table table-hover table-bordered" width="100%">
									<thead>
										<tr>
											...
											<th rowspan="2">${_res.get("replenish.sign")}</th>
											<th rowspan="2">${_res.get('courselib.notSignin')}</th>
											<th rowspan="2">${_res.get("operation")}</th>
										</tr>
									</thead>
									<c:forEach items="${showPages.list}" var="teacherchecke" varStatus="status">
										<tr class="odd" align="center">
											<td>${status.count}</td>
											<td>${teacherchecke.real_name}</td>
											<td>${teacherchecke.yuefen}</td>
											<td>${teacherchecke.zks}</td>
											<td>${teacherchecke.zc}</td>
											<td>${teacherchecke.cd}</td>
											<td>${teacherchecke.bq}</td>
											<td>${teacherchecke.wq}</td>
											<c:if test="${teacherchecke.yijs==0}">
												<td><a href="javascript:void(0)" onclick="toCoursecostPage(${teacherchecke.teacher_id},'${teacherchecke.yuefen}',${teacherchecke.zks},${teacherchecke.zc},${teacherchecke.cd},${teacherchecke.wq},${teacherchecke.bq})" title="计算" >计算</a></td>
											</c:if>
											<c:if test="${teacherchecke.yijs==1}">
												<td>已计算</td>
											</c:if>
											</tr>
									</c:forEach>
	...
	function toCoursecostPage(id,yuefen,zks,zc,cd,bq,wei){
		$.layer({
		    type: 2,
		    shadeClose: true,
		    title: "计算薪资",
		    ...
		    offset:['20px', ''],
		    area: ['900px', '600px'],
		    iframe: {src: '${cxt}/salary/jisuanlist/?id='+id+'&yuefen='+yuefen+'&zks='+zks+'&zc='+zc+'&cd='+cd+'&bq+'+bq+'&wei='+wei}
		});
	}
</script>
