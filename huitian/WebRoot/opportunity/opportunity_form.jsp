...
<body>
			    ...
			    <div class="ibox-title">
					<h5>
						<img alt="" src="/images/img/currtposition.png" width="16" style="margin-top:-1px">&nbsp;&nbsp;<a href="javascript:window.parent.location='/account'">${_res.get('admin.common.mainPage')}</a>
						  &gt; <a href='/opportunity/index'>${_res.get('Opp.Sales.Opportunities')}</a>&gt; 添加机会
				    </h5>
			...
			<div class="col-lg-12" >
				<form id="opportunityForm" action="${cxt }/opportunity/save" method="post">
					<fieldset style="width: 100%; padding-top:15px;">
						...
						<input type="hidden"  id="oppscuserid" value="${opportunity.scuserid }" >
						...
						<div class="ibox float-e-margins">
						   ...
						   <div class="ibox-content">
						      <div>
								...
								<p>
									<label>QQ：</label>
									<input type="text" id="qq" name="opportunity.qq"  <c:if test="${num eq '0'}">disabled</c:if>
									value="${opportunity.qq }" size="20" maxlength="20" vMin="0" vType="qq" onblur="onblurVali(this);" onchange="checkExist('qq')"/>
									<!-- <span id="emailMes" class="spanred"></span> -->
								</p>
								<p>
									<label>微信：</label>
									<input type="text" id="weixin" name="opportunity.weixin"  <c:if test="${num eq '0'}">disabled</c:if>
									value="${opportunity.weixin }" size="20" maxlength="20" vMin="0" vType="qq" onblur="onblurVali(this);" onchange="checkExist('weixin')"/>
									<!-- <span id="emailMes" class="spanred"></span> -->
								</p>
	...
	$(function(){
		...
		function chooseKcge() {
			...
			var oppscuserid = $("#oppscuserid").val();
			var flag = $("#num").val()=='0';
			$.ajax({
				url:'/sysuser/getKcge',
				type:'post',
				data:{'id':id,'oppkcuserid':oppkcuserid,'oppscuserid':oppscuserid},
				dataType:'json',
				success:function(data){
					 var str="";
					 //if(data.oppkcuserid==""){
					...
					str="";
					 //if(data.oppscuserid==""){
						 str+="<option <c:if test='"+flag+"'>disabled</c:if> value=''>请选择市场</option>";
					 //}
					for(var i = 0;i<data.shichangList.length;i++){
						var flag = data.shichangList[i].ID==data.oppscuserid;
						var id = "scuserid"+data.shichangList[i].ID;
						 str+='<option id="'+id+'" <c:if test="'+flag+'">disabled</c:if> value="'+data.shichangList[i].ID+'">'+data.shichangList[i].REAL_NAME+'</option>';
					 }
					$("#scuserid").html(str);
					$("#scuserid"+data.oppscuserid).attr("selected",true);
					$("#scuserid").trigger("chosen:updated");
				}
			});
			chooseSource();
		...
		function getLikeMediator(name){
			var scuserid = $("#scuserid").val();
			if(name.trim()!=""){
				$.ajax({
					url:'/mediator/getLikeMediatorName',
					type:'post',
					data:{'name':name,'scuserid':scuserid},
					dataType:'json',
					success:function(data){
						var str="";
						if(data.code==1){
