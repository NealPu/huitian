							...
							<c:forEach items="${showPages.list}" var="company" varStatus="index">
								<tr align="center">
									...
									<c:if test="${operator_session.qx_companyedit }">
										<a href="#" onclick="updateCompany(${company.id})" style="color: #515151">${_res.get('Modify')}</a>&nbsp;|&nbsp;
									</c:if>
									<c:if test="${operator_session.qx_companydelete }">
										<a href="#" onclick="deleteFunc(${company.id})" style="color: #515151">${_res.get('admin.common.delete')}</a>
									</c:if>
									</td>
								</tr>
							</c:forEach>
