...
<body>
				...
				<div class="ibox float-e-margins">
				    <div class="ibox-title">
						<h5>
						  ...
						  &gt;<a href='/teacher/index?_query.state=0'>销售统计</a> &gt;渠道统计
						</h5>
						<a onclick="window.history.go(-1)" href="javascript:void(0)" class="btn btn-outline btn-primary btn-xs" style="float:right">${_res.get("system.reback")}</a>
          		<div style="clear:both"></div>
					</div>
				<form action="/opportunitycensus/opportunityCensusBymediator" method="post" id="Form">
					  ...
					  <div class="flolet">
							...
							<select id="campus" name="campus" class="chosen-select" style="width: 130px;" tabindex="4">
							   ...
							   <c:forEach items="${campusList}" var="campus">
									<option value="${campus.Id}" class="options"  <c:if test="${campus.Id == campusid}">selected="selected"</c:if>>${campus.CAMPUS_NAME}</option>
								</c:forEach>
							</select>
					   </div>	
						<div class="form-group flolet" id="data_4" style="">
                            <label style="float: left;margin-top:6px">选择月份：</label>
                            <div class="input-group date" style="width:180px;float: left;">
                                <input id="yuefen2" type="text"  name="yuefen2" value="${yuefen2}" class="form-control" value="2015-08">
                                <span class="input-group-addon" style="display: none;"></span>
                            </div>
                            <div style="width:30px;height:30px;line-height:30px;text-align:center;background:#E5E6E7;float: left;">到</div>
                            <div class="input-group date flolet" style="width:180px">
                                <input id="yuefen1" type="text" name="yuefen1" value="${yuefen1}" class="form-control" value="2015-08">
                                <span class="input-group-addon" style="display: none;"></span>
                            </div>
                       </div>
                       <div class="flolet">
                         <input type="submit" value="${_res.get('admin.common.select')}" class="btn btn-outline btn-info" id="" >
					   </div>
					   <div style="clear: both;"></div>
					</div>
				</form>
									...
									<tr align="center">
									 ...
									 <c:forEach items="${monthmap}" var="month" varStatus="status">
										<tr align="center">
											...
											<tr>
												<td>${campus.realname}</td>
												<td>${campus.ZXL}</td>
												<td>${campus.CDL}</td>
												<td>
												  <a href="javascript:void(0)" onclick="tongji('${month.key}',${campus.message})">渠道明细</a>
												</td>
											</tr>
											</c:forEach>
										</tr>
    ...
    <script src="/js/dist/timelineOption_qudao.js"></script>
	<script>
