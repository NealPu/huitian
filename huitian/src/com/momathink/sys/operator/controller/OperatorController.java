...
public class OperatorController extends BaseController {
	...
	public void delthisRole(){
		renderJson(operatorService.deleteThisRole(id));
	}
	...
	public void unloadIcon(){
		try{
			...
			String url = upload.getSaveDirectory();
			String name = upload.getFileName();
			File file = new File(url+name);
			String newurl = url.substring(0,url.length()-7)+"images\\lefticon\\";
			file.renameTo(new File(newurl+name));
			Icon hp = new Icon();
			hp.set("url", newurl+name).set("name", name).set("sysuserid",getSysuserId()).save();
			setAttr("code",0);
			setAttr("url",name);
