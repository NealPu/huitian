...
public class AccountController extends BaseController {
	...
	public void getAccountByNameLike() {
		try {
			...
			String sql = "select * from account where   state=0 "
					...
					+ " and (user_name like \"%" + userName + "%\" or real_name like \"%" + userName + "%\") order by Id desc";
	...
	public void getAccountByClassLike(){
		...
		String sql = "SELECT " +
				...
				"WHERE class_order.classNum LIKE \"%"+classId+"%\" GROUP BY class_order.classNum";
			...
			if(ToolUtils.CheckKeyWord(email==null?"":email.replaceAll(" ", ""))){
				...
				if (account == null) {
					...
					if(ToolMonitor.whetherExpired(new Date())){
						...
						logger.info("授权有效");
						if(Role.isTeacher(account.getStr("roleids"))||Role.isStudent(account.getStr("roleids"))||Role.isDudao(account.getStr("roleids"))||Role.isJiaowu(account.getStr("roleids"))){
	...
	public void teachertongjilog() {
		try {
			...
			List<CoursePlan> teacherlist = CoursePlan.coursePlan.getStatTeacherPlan(starttime,endtime,teacherId);
	...
	public void studenttongjilog() {
		try {
			...
			if (Role.isAdmins(loginAccount.getStr("roleids")) ||Role.isStudent(loginAccount.getStr("roleids"))||Role.isDudao(loginAccount.getStr("roleids"))) {
				...
				}else if(Role.isDudao(loginAccount.getStr("roleids"))){
					condition += " AND s.SUPERVISOR_ID=" + loginUserId;
				} else if(Role.isKcgw(loginAccount.getStr("roleids"))){
					condition += " AND s.campusid in (SELECT  ac.campus_id from account_campus ac WHERE ac.account_id=" + loginUserId+")";
				}else if(Role.isTeacher(loginAccount.getStr("roleids"))){
					condition += " AND cp.teacher_id=" + loginUserId;
				}
				if(!StringUtils.isEmpty(campusid)){
					condition += " and  s.campusid = "+ campusid;
				}
	...
	public synchronized void payFrontMoney(){
		AccountBook accountBook = getModel(AccountBook.class);
		Map<String, String> result = accountService.accountManage(Constants.ACCOUNT_FRONT_MONEY, accountBook.getInt("accountid").toString(), null, null, getSysuserId().toString(), accountBook.getBigDecimal("temprealamount").doubleValue() ,null, null);
		renderJson(result);		
	}
}
