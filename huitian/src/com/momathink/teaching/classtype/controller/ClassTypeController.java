...
public class ClassTypeController extends BaseController {
	...
	private static Logger logger = Logger.getLogger(ClassTypeController.class);
	private ClassTypeService classTypeService = new ClassTypeService();
	private ClassOrderService classOrderService = new ClassOrderService();

	// 班型管理分页
	public void index() {
		classTypeService.list(splitPage);
	...
	public void findClassOrder() {
		try {
			...
			classOrderService.list(splitPage);
			setAttr("showPages", splitPage.getPage());
			List<Subject> subject = Subject.dao.getSubject();
	...
	public void editClassType() {
		String[] str = getPara().split(",");
		String type_id = str[0];// 班型id
		Integer type = Integer.parseInt(str[1]);// 方式，2为修改班型
		...
		try {
			if (classType == null) { // 如果为空
				if ("2".equals(type) && id != "" && id != null && !"0".equals(id)) { // 如果值为2则更新原有班型
					ClassType.dao.findById(id).set("name", name).set("lesson_count", lesson_count).set("subjectids", subids).update();
					...
					new ClassType().set("name", name).set("lesson_count", lesson_count).set("subjectids", subids).save();
					for (int i = 0; i < course_id.length; i++) {
						Record record = ClassType.dao.findRecordCourse(course_id[i]);
				...
				ClassType.dao.findById(classType.getInt("id")).set("NAME", name).set("lesson_count", lesson_count).set("subjectids", subids).update();
				Integer type_id = ClassType.dao.getClassTypeIdByName(name);
	...
	public void editClassOrder() {
		...
		List<Campus> campusList = Campus.dao.find("select * from campus where state = 0");
	...
	public void addClassOrder() {
		List<ClassType> classTypeList = ClassType.dao.getClassTypekeyong();
		...
		try {
			if (!ToolString.isNull(classOrder.getInt("id")+"")) {// 班次已存在则更新
					...
					if (course_id != null && course_id.length > 0) {
						...
						if (account != null) { // 如果不为空则更新
							...
							account.set("EMAIL", classOrder.getStr("classNum")).set("REAL_NAME", classOrder.getStr("classNum"))
									...
									.set("SUBJECT_ID", Util.printArray(sub_id)).set("CLASS_TYPE", Util.printArray(cur_id)).set("USER_PWD", ToolMD5.getMD5("111111"))
				...
				if (course_id != null && course_id.length > 0) {
					...
					account.set("SUBJECT_ID", Util.printArray(sub_id));
					account.set("CLASS_TYPE", Util.printArray(cur_id));
					account.set("USER_PWD", ToolMD5.getMD5("111111"));
	...
	public void addStuToClass() {
		try {
			...
			List<Record> lists = ClassType.dao.getClassCoursePlans(class_id);
			Iterator<Record> cpit = lists.iterator();
			while (cpit.hasNext()) {
				Record cplan = cpit.next();
				if (cplan.getInt("STATE") == 0) {
					cpit.remove();
				}
			}
			Integer temp = 0;
			...
			for (int i = 0; i < sids.length; i++) {
				if (sids[i] != "" && sids[i] != null && sids[i] != " ") {
					...
					/*
					 * ClassType.dao.updateNewStuCourse(Integer.parseInt(sids[
					 * i]),class_id);
					 */
					temp += test;
				}
