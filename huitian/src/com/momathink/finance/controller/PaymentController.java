...
public class PaymentController extends BaseController {
	...
	public void index(){
		paymentService.list(splitPage);
		setAttr("showPages", splitPage.getPage());
		render("/finance/payment_detail.jsp");
	}
	...
	public synchronized void savePayment(){
	...
	public void paymentList(){
		...
		List<Payment> plist = Payment.dao.findByOrderId(orderId);
	...
	public void arrers(){
		try{
				String studentids= CourseOrder.dao.getArrearStudentIds();
				if(!studentids.equals("")){
					List<Student> slist = Student.dao.findByIds(studentids);
					for(Student ss:slist){
						List<CourseOrder> colist  = CourseOrder.dao.findArrearByStudentId(ss.getInt("id").toString());
						if(!colist.isEmpty()){
						...
						ss.put("row",colist.size());
						ss.put("payone",colist.get(0));
						colist.remove(0);
						ss.put("allpay",colist);
					}
					setAttr("list",slist);
				}else{
					setAttr("list","");
				}
			renderJsp("/finance/studentArrers.jsp");
		}catch(Exception ex){
			ex.printStackTrace();
		}
	}
